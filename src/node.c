...
attr_t *new_attr(void) {
  attr_t *attr = arena_alloc(sizeof(attr_t));
  if (attr == NULL) {
    return NULL;
  }
...
attr_t *new_char_attr(attr_key_t key, char *value) {
  ...
  attr->data = (u8_t *)arena_alloc(attr->data_size);
...
attr_t *new_s32_attr(attr_key_t key, s32_t value) {
  ...
  attr->data = arena_alloc(attr->data_size);
  memcpy(attr->data, &value, attr->data_size);
  return attr;
}
...
node_t *new_node(node_type_t type, u32_t attr_count, ...) {
  attr_t **attrs = arena_alloc(attr_count * POINTER_SIZE);
  if (NULL == attrs) {
    return NULL;
  }
